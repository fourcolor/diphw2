...
class Question3:
    ...
    def onBoard(self, text):
        fs = cv2.FileStorage('Q3/alphabet_lib_onboard.txt', cv2.FILE_STORAGE_READ)
        self.word = text
        self.chessboard_AR(fs)

    def Vertical(self, text):
        fs = cv2.FileStorage('Q3/alphabet_lib_vertical.txt', cv2.FILE_STORAGE_READ)
        self.word = text
        self.chessboard_AR(fs)
    ...
    def chessboard_AR(self, fs):
        ...
        images = glob.glob('Q3/Augmented_Reality/*.bmp')
        for fname in images:
            ...
            if ret == True:
                ...
                self.ShowWord(img, alphabet, rvecs, tvecs, mtx, dist)
